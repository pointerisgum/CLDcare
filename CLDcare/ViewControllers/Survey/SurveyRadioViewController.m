...
@interface SurveyRadioViewController ()
...
@property (strong, nonatomic) NDHint *selectedItem;
@end

@implementation SurveyRadioViewController
...
//    temp.title = @"Q2. 평상 시와 다른 불편함을 느끼셨나요?";
...
- (IBAction)goNext:(id)sender {
    ...
    NSArray *ar = @[@{@"inputOrder":@(_order),
                      @"displayOrder":@(_selectedItem.displayOrder)}];
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    NDHint *hintItem = _items[indexPath.row];
    _selectedItem = hintItem;
    
    [self setNextEnable:true];
